...
def load_metrics():
    ...
    for s in m.ALTS:
      ...
      d=pd.concat(acc[s],ignore_index=True).sort_values('time');d['hour']=d.time.dt.floor('h')
      # OI is a stock: last 5m observation; ratios are averaged over the hour.
      h=pd.DataFrame(index=d.hour.drop_duplicates().sort_values())
      h['sum_open_interest_value']=d.groupby('hour').sum_open_interest_value.last()
      h['sum_open_interest']=d.groupby('hour').sum_open_interest.last()
...
def candidates(M):
    ...
    for h in [24,72]:
      # 1) top-trader position vs broad-account divergence: follow informed relative positioning.
      sd=smart.shift(1).rolling(h,min_periods=max(12,h//2)).mean()
      # 2/3) OI-confirmed taker crowding: test contrarian and continuation as fixed economic opposites.
      oig=np.log(oi).diff(h).shift(1);oz=csz(oig);tz=csz(logtak.shift(1).rolling(h,min_periods=max(12,h//2)).mean());build=oz.clip(lower=0)
      contra=-(tz*build);flow=tz*build
      for reb in [8,24]:
        out[f'smart_h{h}_r{reb}']=signal_weights(sd,reb)
        out[f'oi_contra_h{h}_r{reb}']=signal_weights(contra,reb)
        out[f'oi_flow_h{h}_r{reb}']=signal_weights(flow,reb)
    return out
...
def main():
  ...
  C=candidates(M);cres={};ceq={}
  for n,W in C.items():
    eq,cost=m.simulate(W,op,F);ceq[n]=eq;cres[n]={'dev':m.submetrics(eq,m.TRADE_START,m.DEV_END),'full':m.metrics(eq),'holdout1':m.submetrics(eq,m.DEV_END,m.HOLD1_END),'confirm':m.submetrics(eq,m.HOLD1_END,m.CONF_END),'secondary2026':m.submetrics(eq,m.CONF_END,m.END),'corr_rms_dev':corr_eq(eq,rmsEq,m.TRADE_START,m.DEV_END),'corr_fcd_dev':corr_eq(eq,fcdEq,m.TRADE_START,m.DEV_END),'corr_rms_full':corr_eq(eq,rmsEq,m.TRADE_START,m.END),'corr_fcd_full':corr_eq(eq,fcdEq,m.TRADE_START,m.END),'cost':cost}
  eligible=[n for n,v in cres.items() if v['dev']['Sharpe']>0 and abs(v['corr_rms_dev'])<=.40 and abs(v['corr_fcd_dev'])<=.40]
  if eligible: winner=max(eligible,key=lambda n:cres[n]['dev']['Sharpe']*(1-abs(cres[n]['corr_rms_dev']))*(1-abs(cres[n]['corr_fcd_dev'])))
  else:winner=max(cres,key=lambda n:cres[n]['dev']['Sharpe'])
  nw=C[winner];alloc={};target=.95*rdev['CAGR']
  # Locked 5%-step allocation, new alpha/FCD <=30%, RMS >=50%; aggregate before execution. Modest gross scalar <=1.30x.
  for wf in np.arange(0,.31,.05):
   for wn in np.arange(.05,.31,.05):
    ...
    for lev in [1.0,1.1,1.2,1.3]:
      W=base*lev;eq,cost=m.simulate(W,op,F);name=f'R{wr:.2f}_F{wf:.2f}_N{wn:.2f}_L{lev:.1f}'
      alloc[name]={'weights':{'RMS':wr,'FCD':wf,'NEW':wn,'gross_scalar':lev},'dev':m.submetrics(eq,m.TRADE_START,m.DEV_END),'full':m.metrics(eq),'holdout1':m.submetrics(eq,m.DEV_END,m.HOLD1_END),'confirm':m.submetrics(eq,m.HOLD1_END,m.CONF_END),'secondary2026':m.submetrics(eq,m.CONF_END,m.END),'cost':cost}
  ...
  devgoal=[n for n,v in alloc.items() if v['dev']['CAGR']>=target and v['dev']['MaxDD']>-.15];fullgoal=[n for n,v in alloc.items() if v['full']['CAGR']>=.95*rfull['CAGR'] and v['full']['MaxDD']>-.15]
