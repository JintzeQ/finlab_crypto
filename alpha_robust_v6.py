...
def main():
 ...
 sig=signal_for(cl,168,336,24);W,g,t,net,mkt=path_metrics(sig,op,48,ALTS,5)
 out={'fixed':'b168_h336_s24_risk_r48_none','periods':{},'cost_stress':{},'neighbors':{},'leave_one_out':{},'IC':{},'monthly':{},'concentration':{}}
 ...
 for bw in [168,336]:
  for h in [240,336,504]:
   for sk in [0,24]:
    for reb in [24,48,72]:
     ss=signal_for(cl,bw,h,sk);_,gg,tt,nn,mm=path_metrics(ss,op,reb,ALTS,5);key=f'b{bw}_h{h}_s{sk}_r{reb}';out['neighbors'][key]={}
     for n,(a,z) in PERIODS.items():
 ...
 Path('alpha_v6_robust').mkdir(exist_ok=True);json.dump(out,open('alpha_v6_robust/summary.json','w'),indent=2,allow_nan=True);print('===ROBUST===');print(json.dumps(out,indent=2,allow_nan=True),flush=True)
